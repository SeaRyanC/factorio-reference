...
import object_model = require("./om");
import compileAndRun = require("./eval-plugin");
import factorio = require("./factorio-plugin");
...
function fixed(s: number) {
    return s.toFixed(2);
}

function integer(s: number) {
    return commafy(Math.ceil(s));
}

function commafy(n: number) {
...
export async function getConverter(): Promise<Renderer>{
    ...
    const compileAndRunExt = await compileAndRun.getCompilerExtension(globalForEval, loadFile, []/*, extraFiles.map(e => path.join(repoRoot, e))*/);
